...
router.get('/:date', (req, res) => {
    if (!ObjectId.isValid(req.params.date))
        return res.status(400).send('No  record with given date : ${req.params.id}');

    Lab.findById(req.params.date, (err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving Labs :' + JSON.stringify(err, undefined, 2)); }
    })
    ...
    if (Lab.findOne(req.params.date, (err, doc) => {
        if (!err) {
            if (Lab.findOne(req.params.time, (err, doc) => {
                if (!err) {
                    if (Lab.findOne(req.params.lab, (err, doc) => {
                        if (!err) { console.log('found overlap');}
                        else { res.send(doc); console.log('no overlap'); }
                    }));
                }
